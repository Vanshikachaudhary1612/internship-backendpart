...
router.post('/:id/favorite', verifyToken, async (req, res) => {
    try {
        ...
        document.isFavorite = !document.isFavorite; // Toggle favorite status
        await document.save();
        res.json(document);
    } catch (error) {
        res.status(500).json({ message: 'Server error' });
...
router.post('/:id/share', verifyToken, async (req, res) => {
    ...
    try {
        ...
        document.sharedWith.push(recipientEmail);
        await document.save();
...
router.get('/shared-with-me', verifyToken, async (req, res) => {
    try {
        const documents = await Document.find({ sharedWith: req.user.email });
        res.json(documents);
    } catch (error) {
        res.status(500).json({ message: 'Server error' });
    }
